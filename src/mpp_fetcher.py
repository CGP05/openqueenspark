...
def fetch_mpp_profile(mpp: dict) -> dict:
    ...
    try:
        ...
        if response.status_code != 200:
            logger.warning(
                f"Failed to fetch profile for {mpp['name']}: HTTP {response.status_code}"
            )
            return mpp
        ...
        riding_match = re.search(
            r"(?:Riding|Constituency|for\s+)([^,\n]+)", text, re.IGNORECASE
        )
        if riding_match:
        ...
        # Extract photo URL
        photo_url = ""
        img = soup.find("img", alt=re.compile(re.escape(mpp["name"]), re.IGNORECASE))
        if not img:
            img = soup.find(
                "img", class_=re.compile(r"photo|headshot|portrait", re.IGNORECASE)
            )
        if img:
