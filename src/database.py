...
def insert_speaker(name, party_name=None, constituency=None, title=None, email=None, phone=None, website=None, photo_url=None, biography=None, first_elected=None):
    conn = get_connection()
    cursor = conn.cursor()
    ...
    cursor.execute('SELECT id, party_id, constituency, title, email, phone, website, photo_url, biography, first_elected FROM speakers WHERE name = ?', (name,))
    row = cursor.fetchone()
    if row:
        speaker_id = row['id']
        ...
        if email and not row['email']:
            cursor.execute('UPDATE speakers SET email = ? WHERE id = ?', (email, speaker_id))
        if phone and not row['phone']:
            cursor.execute('UPDATE speakers SET phone = ? WHERE id = ?', (phone, speaker_id))
        if website and not row['website']:
            cursor.execute('UPDATE speakers SET website = ? WHERE id = ?', (website, speaker_id))
        if photo_url and not row['photo_url']:
            cursor.execute('UPDATE speakers SET photo_url = ? WHERE id = ?', (photo_url, speaker_id))
        if biography and not row['biography']:
            cursor.execute('UPDATE speakers SET biography = ? WHERE id = ?', (biography, speaker_id))
        if first_elected and not row['first_elected']:
            cursor.execute('UPDATE speakers SET first_elected = ? WHERE id = ?', (first_elected, speaker_id))
        conn.commit()
        conn.close()
        return speaker_id
    ...
    cursor.execute('''
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', (name, party_id, constituency, title, email, phone, website, photo_url, biography, first_elected))
    conn.commit()
...
# NEW: Bills functions
def insert_bill(bill_number, bill_name=None, short_title=None, sponsor_name=None, 
                sponsor_party=None, status=None, stage=None, introduced_date=None,
                last_activity_date=None, session_id=None, parliament=44, session_number=1,
                legisinfo_id=None, description=None):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute('SELECT id FROM bills WHERE bill_number = ? AND session_id = ?', (bill_number, session_id))
    row = cursor.fetchone()
    if row:
        # Update existing
        cursor.execute('''
            ...
            WHERE id=?
        ''', (bill_name, short_title, sponsor_name, sponsor_party, status, stage, 
              last_activity_date, description, row['id']))
        conn.commit()
        bill_id = row['id']
    else:
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (bill_number, bill_name, short_title, sponsor_name, sponsor_party, status, stage,
              introduced_date, last_activity_date, session_id, parliament, session_number,
              legisinfo_id, description))
        conn.commit()
...
def add_bill_stage(bill_id, stage, date=None, details=None, vote_yes=None, vote_no=None, vote_abstain=None):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute('''
...
def get_bill_by_id(bill_id):
    ...
    if bill:
        cursor.execute('SELECT * FROM bill_stages WHERE bill_id = ? ORDER BY date', (bill_id,))
...
def insert_committee(name, short_name=None, type=None, chair_name=None, vice_chair_name=None,
                     clerk_name=None, meeting_schedule=None, website=None):
    conn = get_connection()
    cursor = conn.cursor()
    ...
    if row:
        cursor.execute('''
            ...
            WHERE id=?
        ''', (short_name, type, chair_name, vice_chair_name, clerk_name, meeting_schedule, website, row['id']))
        conn.commit()
        committee_id = row['id']
    else:
        cursor.execute('''
            INSERT INTO committees (name, short_name, type, chair_name, vice_chair_name,
                clerk_name, meeting_schedule, website)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (name, short_name, type, chair_name, vice_chair_name, clerk_name, meeting_schedule, website))
        conn.commit()
...
def insert_committee_meeting(committee_id, meeting_date, meeting_number=None, title=None,
                             evidence_text=None, witnesses=None, documents=None, source_url=None):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute('''
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ''', (committee_id, meeting_date, meeting_number, title, evidence_text,
          json.dumps(witnesses or []), json.dumps(documents or []), source_url))
    conn.commit()
...
def index_speech_for_search(speech_id, speaker_name, party_name, text, h2_heading, h3_heading, session_date):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute('''
        INSERT OR REPLACE INTO speeches_fts (speech_id, speaker_name, party_name, text, h2_heading, h3_heading, session_date)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    ''', (speech_id, speaker_name, party_name or '', text, h2_heading or '', h3_heading or '', session_date))
    conn.commit()
    conn.close()


def index_bill_for_search(bill_id, bill_number, bill_name, short_title, sponsor_name, description):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute('''
        INSERT OR REPLACE INTO bills_fts (bill_id, bill_number, bill_name, short_title, sponsor_name, description)
        VALUES (?, ?, ?, ?, ?, ?)
    ''', (bill_id, bill_number, bill_name or '', short_title or '', sponsor_name or '', description or ''))
    conn.commit()
    conn.close()
...
def rebuild_search_indexes():
    ...
    for row in cursor.fetchall():
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (row['id'], row['speaker_name'], row['party_name'] or '', row['text'], 
              row['h2_heading'] or '', row['h3_heading'] or '', row['session_date']))
    
    # Clear and rebuild bills index
    cursor.execute('DELETE FROM bills_fts')
    cursor.execute('SELECT id, bill_number, bill_name, short_title, sponsor_name, description FROM bills')
    for row in cursor.fetchall():
        cursor.execute('''
            INSERT INTO bills_fts (bill_id, bill_number, bill_name, short_title, sponsor_name, description)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (row['id'], row['bill_number'], row['bill_name'] or '', row['short_title'] or '', 
              row['sponsor_name'] or '', row['description'] or ''))
